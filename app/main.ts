...
function initIPC() {
  ...
  ipcMain.on('send-mail-request', (event, arg) => {
    console.log(
      'Preparing mail: ' + LocalConfigMain.getInstance().getMailProcessor()
    );
    ...
    require('child_process').exec(
      cmd,
      function (err, stdout: string, stderr: string): void {
  ...
  ipcMain.on('select-folder-request', (event, arg) => {
    try {
      ...
      pathPromise.then(path => {
        if (path.canceled) event.reply('select-folder-reply', '');
        else event.reply('select-folder-reply', path.filePaths[0]);
      });
    } catch (e) {
  ...
  ipcMain.on('select-files-request', (event, arg) => {
    try {
      ...
      pathPromise.then(path => {
        if (path.canceled) event.reply('select-files-reply', []);
        else {
  ...
  ipcMain.on('show-tray-balloon-request', (event, arg) => {
    ...
    const image =
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      require('electron').nativeImage.createFromPath(eisensteckenIconPng);
    tray.displayBalloon({
      icon: image,
      iconType: 'custom',
      title: arg[0],
