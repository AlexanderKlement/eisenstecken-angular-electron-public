  ...
  implements OnInit, OnDestroy
{
  ...
  lockFunction = (api: DefaultService, id: number): Observable<Lock> =>
    api.islockedJobJobIslockedJobIdGet(id);
  dataFunction = (api: DefaultService, id: number): Observable<Job> =>
    api.readJobJobJobIdGet(id);
  unlockFunction = (api: DefaultService, id: number): Observable<boolean> =>
    api.unlockJobJobUnlockJobIdPost(id);

  ngOnInit(): void {
    super.ngOnInit();
    this.initJobGroup();
    this.users$ = this.api
      .readUsersUsersGet(0, '', 100, true)
      .pipe(map(users => users.filter(user => user.office)));
    if (this.createMode) {
  ...
  observableReady(): void {
    ...
    if (!this.createMode) {
      this.data$
        .pipe(
          tap(job => this.jobGroup.patchValue(job)),
          first()
        )
        .subscribe(job => {
  ...
  private onSubmitSubJobCreate(): void {
    ...
    this.api
      .addSubjobToJobJobSubJobJobIdPost(this.mainJobId, subJobCreate)
      .subscribe(
        job => {
  ...
  private onSubmitMainJobEdit(): void {
    ...
    this.api.updateJobJobJobIdPut(this.id, jobUpdate).subscribe(
      job => {
        this.createUpdateSuccess(job);
      },
      error => {
  ...
  private onSubmitSubJobEdit(): void {
    ...
    this.api.updateSubjobJobSubJobSubjobIdPut(this.id, subJobCreate).subscribe(
      job => {
        this.createUpdateSuccess(job);
      },
      error => {
