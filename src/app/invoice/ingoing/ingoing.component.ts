...
export class IngoingComponent implements OnInit {
  ...
  buttons: TableButton[] = [
    {
      name: (condition: any) =>
        condition ? 'Zahlung entfernen' : ' Zahlung hinzufügen',
  ...
  paidClicked(event: any, id: number) {
    ...
    this.api
      .readIngoingInvoiceIngoingInvoiceIngoingInvoiceIdGet(id)
      .pipe(first())
      .subscribe(ingoingInvoice => {
        let text = `Die Rechnung ${ingoingInvoice.number} vom ${moment(
          ingoingInvoice.date,
          'YYYY-MM-DD'
        ...
        if (!ingoingInvoice.paid) {
          ...
          returnFunction = result => {
            if (result) {
              this.api
                .payIngoingInvoicePaymentIngoingInvoiceIdPayPost(id)
                .pipe(first())
                .subscribe(() => {
                  this.loadTables();
                });
          ...
          returnFunction = result => {
            if (result) {
              this.api
                .unpayIngoingInvoicePaymentIngoingInvoiceIdUnpayPost(id)
                .pipe(first())
                .subscribe(() => {
                  this.loadTables();
                });
  ...
  private initAllIngoingInvoiceDataSource(): void {
    this.allIngoingInvoiceDataSource = new TableDataSource(
      ...
      (api, filter, sortDirection, skip, limit) =>
        api.readIngoingInvoicesIngoingInvoiceGet(
          skip,
          limit,
          filter,
          undefined,
          ...
          rows.push({
            ...
            route: () => {
              this.router.navigateByUrl(
                '/invoice/ingoing/' + dataSource.id.toString()
              );
            },
          });
        });
      ...
      api =>
        api.countIngoingInvoicesIngoingInvoiceCountGet(
          undefined,
          this.selectedYear
        )
    );
  ...
  private initPaidIngoingInvoiceDataSource(): void {
    this.paidIngoingInvoiceDataSource = new TableDataSource(
      ...
      api =>
        api.countIngoingInvoicesIngoingInvoiceCountGet(true, this.selectedYear)
    );
    this.paidIngoingInvoiceDataSource.loadData();
  }
  ...
  private initUnPaidIngoingInvoiceDataSource(): void {
    this.unPaidIngoingInvoiceDataSource = new TableDataSource(
      ...
      api =>
        api.countIngoingInvoicesIngoingInvoiceCountGet(false, this.selectedYear)
    );
    this.unPaidIngoingInvoiceDataSource.loadData();
  }
  ...
  private deleteClicked($event: any, id: number) {
    ...
    this.api
      ...
      .pipe(first())
      .subscribe(ingoingInvoice => {
        const text = `Die Rechnung ${ingoingInvoice.number} vom ${moment(
          ingoingInvoice.date,
          'YYYY-MM-DD'
        ...
        const returnFunction = result => {
          if (result) {
            this.api
              .deleteIngoingInvoiceIngoingInvoiceIngoingInvoiceIdDelete(id)
              .pipe(first())
              .subscribe(() => {
                this.loadTables();
              });
