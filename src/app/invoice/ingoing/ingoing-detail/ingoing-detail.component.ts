  ...
  initDescriptiveArticleTable(): void {
    this.descriptiveArticleSource = new TableDataSource<DescriptiveArticle>(
      ...
      (api: DefaultService, filter, sortDirection, skip, limit) =>
        api.readIngoingInvoicesIngoingInvoiceArticlesIngoingInvoiceIdGet(
          this.id,
          skip,
          limit,
          filter
        ...
        dataSourceClasses.forEach(dataSource => {
          rows.push({
            values: {
              ...
              single_price: formatCurrency(
                dataSource.single_price,
                'de-DE',
                'EUR'
              ),
              ...
              // eslint-disable-next-line @typescript-eslint/naming-convention
              total_with: formatCurrency(
                dataSource.single_price *
                  dataSource.amount *
                  (1 - dataSource.discount / 100) *
                  (1 + dataSource.vat.amount / 100),
                'de-DE',
                'EUR'
              ),
              // eslint-disable-next-line @typescript-eslint/naming-convention
              total_without: formatCurrency(
                dataSource.single_price *
                  dataSource.amount *
                  (1 - dataSource.discount / 100),
                'de-DE',
      ...
      api =>
        api.readIngoingInvoicesIngoingInvoiceArticlesCountIngoingInvoiceIdGet(
          this.id
        )
    );
