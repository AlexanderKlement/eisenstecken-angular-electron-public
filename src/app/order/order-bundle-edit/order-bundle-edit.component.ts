  ...
  implements OnInit
{
  ...
  lockFunction = (api: DefaultService, id: number): Observable<Lock> =>
    api.islockedOrderBundleOrderBundleIslockedOrderBundleIdGet(id);

  dataFunction = (api: DefaultService, id: number): Observable<OrderBundle> =>
    api.readOrderBundleOrderBundleOrderBundleIdGet(id);

  unlockFunction = (api: DefaultService, id: number): Observable<boolean> =>
    api.unlockOrderBundleOrderBundleUnlockOrderBundleIdPost(id);
  ...
  loadOrders(): void {
    this.api
      .readOrdersByOrderBundleOrderBundleOrdersOrderBundleIdGet(
        this.orderBundleId
      )
      .pipe(first())
      .subscribe(orders => {
      ...
      articleFormArray.push(
        new UntypedFormGroup({
          ...
          unit: new UntypedFormControl(
            article.ordered_unit.name.translation_de
          ),
  ...
  onSubmit(): void {
    ...
    for (const order of this.getOrderFormArray().controls) {
      for (const article of (order.get('articles') as UntypedFormArray)
        .controls) {
        priceUpdate.push({
    ...
    this.api
      .updateOrderedArticlePriceOrderedArticlePricePut(priceUpdate)
      .pipe(first())
      .subscribe(result => {
        if (result) {
          this.router.navigateByUrl(
            'order_bundle/' + this.orderBundleId.toString()
          );
        } else {
        ...
        this.api
          ...
          .subscribe(success => {
            if (success) {
              ...
              this.snackbar.open(
                'Der Artikel konnte leider nicht gelöscht werden. Bitte probieren sie es später erneut',
                'Ok',
                {
                  duration: 10000,
                }
