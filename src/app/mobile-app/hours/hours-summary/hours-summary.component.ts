...
export class HoursSummaryComponent implements OnInit, OnDestroy {
  ...
  refreshAdditionalWorkloadString(): void {
    const additionaWorkloadMinutes = parseInt(
      this.jobFormGroup.get('additionalJob').get('minutes').value,
      10
    );
    if (additionaWorkloadMinutes > 0) {
      const description = this.jobFormGroup
        .get('additionalJob')
        .get('description').value;
      this.additionalWorkloadString =
        description +
        ': ' +
        this.getHoursStringFromMinutes(additionaWorkloadMinutes);
    }
  }
  ...
  refreshMaintenanceString(): void {
    ...
    this.maintenanceString =
      'Instandhaltung: ' + this.getHoursStringFromMinutes(minutes);
  }
  ...
  private refreshEatingPlace(): void {
    const eatingPlaceId = parseInt(
      this.mealFormGroup.get('eatingPlaceId').value,
      10
    );
    this.api
      .getEatingPlacesEatingPlaceEatingPlaceIdGet(eatingPlaceId)
      .pipe(first())
      .subscribe(eatingPlace => {
        if (eatingPlace) {
  ...
  private refreshDrives(): void {
    ...
    for (const drive of this.getDrives().controls) {
      ...
      carObservables$.push(
        this.api.getCarCarCarIdGet(carId).pipe(
          map(car => {
            const driveString =
              drive.get('reasonString').value +
              ': ' +
              car.name +
              ': ' +
              drive.get('km').value +
