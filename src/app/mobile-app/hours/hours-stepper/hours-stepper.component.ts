...
export class HoursStepperComponent implements OnInit {
  ...
  private static initSingleJob(
    minutes: number,
    minutesDirection: number,
    job: Job
  ): UntypedFormGroup {
  ...
  getWorkedHoursString(): string {
    ...
    const minutes = parseInt(
      this.hourFormGroup.get('showingMinutes').value,
      10
    );
  ...
  getAvailableHoursString(): string {
    const spendableMinutes = parseInt(
      this.jobFormGroup.get('spendableMinutes').value,
      10
    );
  ...
  addMinutesToJob(
    ...
    const actualMinutes = parseInt(
      this.getJobs(jobEnum).at(index).get(fieldName).value,
      10
    );
    if (actualMinutes + minutesToAdd < 0) {
      return;
    }
    this.getJobs(jobEnum)
      .at(index)
      .get(fieldName)
      .setValue(actualMinutes + minutesToAdd);
    this.refreshSpentMinutes();
  }
  ...
  getMinutesStringFromJob(
    ...
    return HoursStepperComponent.generateHourString(
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        if (result.jobId !== undefined) {
          this.api
            .readJobJobJobIdGet(result.jobId)
            .pipe(first())
            .subscribe(job => {
              this.getDrives().push(
                this.initDrive(
  ...
  submitClicked() {
    ...
    if (this.date !== undefined && this.userId !== undefined) {
      this.api
        .createWorkDayWorkDayUserIdPost(
          this.userId,
          formatDateTransport(this.date.toDateString()),
          workDayCreate
        )
        .pipe(first())
        .subscribe(() => {
          ...
          this.router.navigateByUrl(
            '/employee/redirect/' + this.userId.toString(),
            { replaceUrl: true }
          );
        });
    } else {
      this.api
        .createWorkDayOwnWorkDayOwnPost(workDayCreate)
        .pipe(first())
        .subscribe(() => {
          this.navigation.removeLastUrl();
          this.router.navigateByUrl('/mobile/hours/redirect', {
            replaceUrl: true,
          });
        });
    }
  ...
  createWorkDayCreate(): WorkDayCreate {
    ...
    for (const jobSectionLists of this.getAllJobs()) {
      for (const jobSection of jobSectionLists.controls) {
        if (
          parseInt(jobSection.get('minutes').value, 10) > 0 ||
          parseInt(jobSection.get('minutesDirection').value, 10) > 0
        ) {
          jobSectionsCreates.push({
            minutes: parseInt(jobSection.get('minutes').value, 10),
            // eslint-disable-next-line @typescript-eslint/naming-convention
            minutes_direction: parseInt(
              jobSection.get('minutesDirection').value,
              10
            ),
            // eslint-disable-next-line @typescript-eslint/naming-convention
    ...
    const minutesMaintenance = parseInt(
      this.jobFormGroup.get('maintenanceMinutes').value,
      10
    );
    ...
    if (
      parseInt(
        this.jobFormGroup.get('additionalJob').get('minutes').value,
        10
      ) > 0
    ) {
      additionalWorkloadCreates.push({
        ...
        description: this.jobFormGroup.get('additionalJob').get('description')
          .value,
      });
    }
    ...
    return {
      ...
      drives: driveCreates,
      // eslint-disable-next-line @typescript-eslint/naming-convention
      eating_place_id: parseInt(
        this.mealFormGroup.get('eatingPlaceId').value,
        10
      ),
      // eslint-disable-next-line @typescript-eslint/naming-convention
  ...
  openDialogClicked(
    ...
    const data = {
      ...
      additionalJobs: this.jobFormGroup.get(
        'additionalJob'
      ) as UntypedFormGroup,
      variant,
  ...
  private refreshShownHoursMinutes(): void {
    ...
    this.hourFormGroup
      .get('showingHours')
      .setValue(Math.trunc(actualMinutes / 60));
  ...
  private refreshSpentMinutes(): void {
    ...
    const spendableMinutes =
      parseInt(this.hourFormGroup.get('minutes').value, 10) - spendMinutes;
    ...
    this.jobs$.pipe(first()).subscribe(jobs => {
      for (const job of jobs) {
        if (job.status.status === 'JOBSTATUS_ACCEPTED') {
          this.getJobs(JobEnum.accepted).push(
            HoursStepperComponent.initSingleJob(0, 0, job)
          );
        }
        if (job.status.status === 'JOBSTATUS_CREATED') {
          this.getJobs(JobEnum.created).push(
            HoursStepperComponent.initSingleJob(0, 0, job)
          );
        }
      }
    ...
    this.authService
      ...
      .subscribe(currentUser => {
        if (currentUser.office) {
          this.jobs$ = this.api.readJobsJobGet(
            0,
            200,
            '',
            undefined,
            'JOBSTATUS_ACCEPTED, JOBSTATUS_CREATED',
            true
          );
        } else {
          this.jobs$ = this.api.readJobsJobGet(
  ...
  private fillData() {
    ...
    this.mealFormGroup
      .get('eatingPlaceId')
      .setValue([this.workDay.eating_place.id]);

    if (this.workDay.additional_workloads.length > 0) {
      const additionalJobGroup = this.jobFormGroup.get('additionalJob');
      additionalJobGroup
        .get('minutes')
        .setValue(this.workDay.additional_workloads[0].minutes);
      additionalJobGroup
        .get('description')
        .setValue(this.workDay.additional_workloads[0].description);
    }
    ...
    for (const jobControlList of this.getAllJobs()) {
      for (const jobControl of jobControlList.controls) {
        ...
        for (const jobSection of this.workDay.job_sections) {
          if (!jobSection.job) {
            this.jobFormGroup
              .get('maintenanceMinutes')
              .setValue(jobSection.minutes);
            continue;
          }
          if (
            jobSection.job.id === parseInt(jobControl.get('jobId').value, 10)
          ) {
            jobControl.get('minutes').setValue(jobSection.minutes);
            jobControl
              .get('minutesDirection')
              .setValue(jobSection.minutes_direction);
          }
        }
      }
    }
