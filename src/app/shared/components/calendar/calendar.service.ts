...
export class CalendarService implements OnDestroy {
  ...
  constructor(
    ...
    this.notificationInterval = setInterval(
      () => {
        this.api
          .readNextCalendarEntriesInCalendarNextTimeMinutesGet(
            this.notificationIntervalDistanceMinutes
          )
          .pipe(first())
          .subscribe(calendarEntries => {
            for (const calendarEntry of calendarEntries) {
              if (
                this.calendarEntriesAlreadyNotified.includes(calendarEntry.id)
              ) {
                continue;
              }
  ...
  public refreshCalendar(calendarId: number, day: number): void {
    const calendarDayListElement = this.findCalendarAndDay(calendarId, day);
    if (
      calendarDayListElement === undefined ||
      calendarDayListElement.subscriber === undefined
    ) {
    ...
    this.api
      .readCalendarEntriesByDayCalendarCalendarsCalendarIdGet(calendarId, day)
      .pipe(first())
      .subscribe(calendarEntries => {
  ...
  private calculateIntervalMilliSeconds(): number {
    return Math.floor(
      this.minutesBetweenUpdate * 60 * 1000 +
        Math.random() * this.randomMaxSeconds
    );
  }

  private findCalendarAndDay(
    calendarId: number,
    day: number
  ): CalendarDayListElement | undefined {
